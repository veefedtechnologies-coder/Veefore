...
async function validateEndpointAvailability() {
  ...
  try {
    ...
    const orderCheck = await axios.get('http://localhost:3000/api/subscription/create-order').catch(err => {
    ...
    const upgradeCheck = await axios.get('http://localhost:3000/api/subscription/upgrade').catch(err => {
      if (err.response?.status === 405) {
