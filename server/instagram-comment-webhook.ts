...
export class InstagramCommentWebhookHandler {
  ...
  private async sendInstagramDirectMessage(
    ...
    try {
      ...
      const response = await fetch(`https://graph.instagram.com/v23.0/${socialAccount.accountId}/messages`, {
        method: 'POST',
        headers: {
  ...
  private async sendDynamicMessage(
    ...
    try {
      ...
      const response = await fetch(
        `https://graph.instagram.com/v23.0/${socialAccount.accountId}/messages?access_token=${pageAccessToken}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
